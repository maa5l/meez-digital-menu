...
function AdminNavLinks({ className, inline }: { className?: string; inline?: boolean }) {
  return (
    <nav className={className} aria-label="قائمة الإدارة">
      {navItems.map((item) => (
        <NavLink
          key={item.to}
          to={item.to}
          ...
          className={({ isActive }) =>
            `${
              inline
                ? "inline-flex shrink-0 whitespace-nowrap items-center gap-2 px-3 py-2 rounded-lg text-sm"
        ...
        >
          <item.icon className={inline ? "w-4 h-4 shrink-0" : "w-5 h-5 shrink-0"} />
          {item.label}
        </NavLink>
      ))}
    </nav>
  ...
  return (
    <div className="min-h-screen bg-secondary/40 flex" dir="rtl">
      <aside className="hidden lg:flex w-72 bg-card border-l border-border flex-col fixed inset-y-0 right-0">
        ...
        <AdminNavLinks className="flex-1 p-4 space-y-1" />
      ...
      <main className="flex-1 lg:mr-72">
        <header className="bg-card border-b border-border px-6 py-5 space-y-4">
          ...
          <AdminNavLinks className="lg:hidden flex gap-2 overflow-x-auto pb-1" inline />
        </header>
